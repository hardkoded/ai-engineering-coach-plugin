...
import { rpc, COLORS, Chart, trackChart, destroyCharts, HARNESS_COLORS } from './shared';
...
/* Harness colors. Keyed by the canonical `Session.harness` values, so it shares the
 * one map in `shared.ts` rather than a second copy that drifted to display names. */
function hc(h: string): string { return HARNESS_COLORS[h] || COLORS.muted; }
